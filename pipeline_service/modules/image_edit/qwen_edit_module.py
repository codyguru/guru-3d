...
class QwenEditModule(QwenManager):
    ...
    def _get_model_pipe(self, transformer, scheduler):
        """Create and return the Qwen image edit pipeline."""
    ...
    def _run_model_pipe(self, seed: Optional[int] = None, **kwargs):
        """Run the model pipeline with given parameters."""
        if seed:
    ...
    def edit_image(self, prompt_image: Image.Image, seed: int) -> Image.Image:
        ...
        try:
            ...
            logger.success(
                f"Edited image generated in {generation_time:.2f}s, "
                f"Size: {image_edited.size}, Seed: {seed}"
            )
